...
class Security extends Actor {
	...
	var _line : Int = 0
			...
			def this(_jail : ActorRef, _system : ActorRef, line : Int) = {
				this()
				this._jail = _jail
			...
			def checkPerson(isBody : Boolean, psgr : Passenger, result : Boolean) {
				...
				isBody match {
				case false =>
				println("Security: Received bag")
				_bags += psgr -> result
				case true =>
				println("Security: Received passenger")
            ...
            println("Security: Passenger sent to jail")
			...
			def tryQuit() {
				_poisonPillCnt += 1
						...
						if (_poisonPillCnt >= 2) {
              println("Security: PoisonPill -> Shutting down")
              _jail ! PoisonPill(true)
							context.stop(self)
						}
